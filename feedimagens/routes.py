...
from feedimagens import app, database, bcrypt
from flask_login import login_required, login_user, logout_user, current_user
from feedimagens.forms import FormLogin, FormCriarConta, FormFoto
...
def perfil(id_usuario):
    if int(id_usuario) == int(current_user.id):
        ...
        if form_foto.validate_on_submit():
            ...
            foto_post = Foto(imagem=nome_seguro, id_usuario=int(current_user.id))
            database.session.add(foto_post)
            database.session.commit()
